...
void test_yaml() {
    YAML::Node root = YAML::LoadFile("/root/workspace/sylar/bin/conf/log.yml");
    //print_yaml(root, 0);
...
#define XX_M(g_var, name, prefix) \
    ...
    YAML::Node root = YAML::LoadFile("/root/workspace/sylar/bin/conf/test.yml");
...
void test_log() {
    ...
    sylar::Config::LoadFromYaml(root);
    std::cout << "-----------------------------" << std::endl;
    std::cout << sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;  //配置后的yaml反序列化的结果
    std::cout << "-----------------------------" << std::endl;
    std::cout << root << std::endl;     //原始的yaml
    SYLAR_LOG_INFO(system_log) << "hello system" << std::endl;
